#!/usr/bin/env python3
"""
Script Name: generate_battle_dashboard_v2.py
Description: Generates the Keimola RF Battle Dashboard.
             v1.1.0 Fix: Points the 'host' variable query to 'rf_battle_stats'
             to ensure the dropdown menu matches the data tags.
Author:      System Architect (Gemini)
"""

import json

FILENAME = "rf_battle_dashboard_v2.json"
TITLE = "Keimola Grid: RF Battle Command v2"
UID = "keimola_rf_battle_v2"
    ...
    return {
      ...
      "templating": {
        "list": [
          ...
          # --- THE FIX IS HERE ---
          # We now query 'rf_battle_stats' for the host list, not 'readsb'.
          # This ensures the dropdown only shows the active nodes (Office/Balcony).
          {
            "allValue": None,
            "current": {},
            "datasource": { "type": "influxdb", "uid": "${DS}" },
            "definition": "SHOW TAG VALUES FROM \"rf_battle_stats\" WITH KEY = \"host\"",
            "hide": 0,
            "includeAll": True,
            "label": "Node",
            "multi": True,
            "name": "host",
            "options": [],
            "query": "SHOW TAG VALUES FROM \"rf_battle_stats\" WITH KEY = \"host\"",
            "refresh": 1,
            "regex": "",
            "skipUrlSync": False,
            "sort": 1,
